...
while True:
    ...
    if np.any(frame):
        ...
        mask = np.repeat(mask[:, :, np.newaxis], channels, axis=2)
        mask1 = np.repeat(mask1[:, :, np.newaxis], channels, axis=2)
        mask2 = np.repeat(mask2[:, :, np.newaxis], channels, axis=2)

        # Resize the image as per the frame capture size
        bg = cv2.resize(bg_image, (height, width))
        bg1 = cv2.resize(bg_image, (height, width))
        bg2 = cv2.resize(bg_image, (height, width))

        bg[mask] = frame[mask]
        bg1[mask1] = frame[mask1]
        bg2[mask2] = frame[mask2]

        frame = bg + bg1 + bg2
        ax1.set_title("cropped object video")

        # Set the data of the two images to frame and mask values respectively
        im1.set_data(frame)
        im2.set_data(mask * (255, 0, 255) + mask1 * (0, 255, 0) + mask2 * (255, 255, 0))
